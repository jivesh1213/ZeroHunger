<%-- 
    Document   : shelterRegistration
    Created on : 25 Jan, 2020, 6:23:31 PM
    Author     : lenovo
--%>
...
<title>Rain Basera Registration</title>
...
#forms{
  margin-top: -20px;
  background-image: url("Pictures/RainBbg.png");
...
<div id="forms">
    <form method="post" action="shelterServlet" style="padding-top: 16%;padding-left: 28.5%; font-size: 17px; font-family: Helvetica; ">
		<input class="t_input" style="width: 630px" type="text" name="r_name" placeholder="Rain Basera Name" <c:if test="${(\"\").equals(shelter)}">id="filter"</c:if> <c:if test="${!(\"\").equals(shelter)}">value="${shelter}"</c:if>/><br/>
		...
		<input class="t_input" style="width: 247px" type="text" name="p_name" placeholder="Contact Person." <c:if test="${(\"\").equals(pName)}">id="filter"</c:if> <c:if test="${!(\"\").equals(pName)}">value="${pName}"</c:if>>
		<input class="t_input" style="width: 247px" type="text" name="phone" placeholder="Contact No." <c:if test="${(\"\").equals(phone)}">id="filter"</c:if> <c:if test="${!(\"\").equals(phone)}">value="${phone}"</c:if>>
                <input class="t_input" style="width: 97px" type="text" name="capacity" placeholder="Capacity" <c:if test="${(\"\").equals(capacity)}">id="filter"</c:if> <c:if test="${!(\"\").equals(capacity)}">value="${capacity}"</c:if>/><br/>
		<input class="t_input" style="width: 630px" type="text" name="email" placeholder="Email ID" <c:if test="${(\"\").equals(email)}">id="filter"</c:if> <c:if test="${!(\"\").equals(email)}">value="${email}"</c:if>/><br/>
                ...
                <input class="largerCheckbox" style="margin-top: 10px; margin-left: 15px" type="checkbox" name="agree" <c:if test="${check != null}">id='col'</c:if>/> I agree to follow all the rules specified by ZeroHunger.
                <br>
                <c:if test="${check != null}">
                    <div style="color: #fa2e02; font-size: 15px; margin-left: 45px"> ${check}</div>
                </c:if><br>
                    <input type="submit" id="submit" style="font-family: Helvetica; font-size: 18px" value="Submit">
	</form>
