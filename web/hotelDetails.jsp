...
<body >
    ...
    <div style="background-image: url(Pictures/Hotelslistback.png); background-size: cover; height: 900px; position: fixed; width: 100%;z-index: -1;margin-top: -145px  "> </div>
        <div style=" width: 100%;margin-top: -95px; margin-left: auto; margin-right: auto; font-family: helvetica; height: 700px">
            <table  style="width: 100%; text-align: center; line-height: 21px; font-size: 17px; font-family: helvetica; border-style: hidden !important">
                  <tr style=" font-size: 20px; font-weight: bold; font-family: serif; background-color: #007dba; font-family: helvetica ">
                    ...
                    <td style="width: 29%">Name of Hotel</td>
                    <td style="width: 25%">Location</td>
                    <td style="width: 20%">Delivery Boy Alloted</td>
                    <td style="width: 21%">Contact Person Role and Number</td>
                </tr>
                <c:forEach var="i" begin="0" end="${hotelList.size() - 1}">
                    <tr style="<c:if test="${i % 2 != 0}">background-color: rgba(250, 247, 246, 0.5)
;</c:if><c:if test="${i % 2 == 0}">background-color: rgba(250, 247, 246, 0.79)</c:if>">
                    <td style="width: 5%">${i + 1}</td>
                    <td style="width: 29%">${hotelList.get(i).getOrgName()}</td>
                    <td style="width: 25%">${hotelList.get(i).getArea()}, ${hotelList.get(i).getCity()} - ${hotelList.get(i).getPincode()}</td>
                    <td style="width: 20%"><c:forEach var="j" begin="0" end="${hotelList.size() - 1}"><c:if test="${boysList.get(j).getOrgname().equals(hotelList.get(i).getOrgName())}">${hotelList.get(j).getOrgName()}<br></c:if></c:forEach></td>
                    <td style="width: 21%">${hotelList.get(i).getpName()}, ${hotelList.get(i).getPhone()}</td>
                </tr>
                </c:forEach>
            </table>
        </div>
